...
class SettingsAPI {
	...
	function callback_select2( $args ) {
		...
		$value = esc_attr( $this->get_option( $args['id'], $args['section'], $args['std'] ) );
		$size  = isset( $args['size'] ) && ! is_null( $args['size'] ) ? $args['size'] : 'regular';
		$html  = sprintf( '<select class="chosen-select" name="%2$s[%3$s]" id="%2$s[%3$s]" data-placeholder="This" data-key="%3$s" data-value="" >', $size, $args['section'], $args['id'] );


		$types = [ 'part', 'assembly', 'product' ];


		foreach ( $types as $type ) {
			//
			//			if ( get_transient( $type . '_data' ) !== null ) {
			//				$posts = get_transient( $type . '_data' );
			//				echo 'trans';
			//			} else {
			...
			$html .= '<optgroup label="' . strtoupper( $type ) . '">';
	...
	function callback_file( $args ) {
		...
		$html = sprintf( '<input type="text" class="%1$s-text wpsa-url" id="%2$s[%3$s]" name="%2$s[%3$s]"  data-key="%2$s" data-value="" value="%4$s"/>', $size, $args['section'], $args['id'], $value );
		$html .= '<input type="button" class="button wpsa-browse" value="' . $label . '" />';
		$html .= $this->get_field_description( $args );

		$icon = plugins_url( 'assets/images/icons/48px/ai.png', dirname( __FILE__ ) );
	...
	function wco_ajax() {
		...
		$idd = $_POST['ajaxid'];
		$aj  = $_POST['bom_ajax_text'];


		$id = $_POST['select2'];
		//update_option( 'bom_settings', $aj );


		require __DIR__ . '/PostObject.php';
		$obj = new PostObject( $id );
